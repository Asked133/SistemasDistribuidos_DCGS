...
class InventoryService(diablo_inventory_pb2_grpc.InventoryServiceServicer):
    ...
    def CreateBulkLoot(self, request_iterator, context):
        ...
        for req in request_iterator:
            ...
            try:
                ...
                # Always return the fully populated item (with generated ID) to the client
        ...
        response = diablo_inventory_pb2.BulkCreateResponse(
            items_creados=count,
            mensaje="Loot masivo almacenado correctamente"
        )
        response.items.extend(created_items)
        return response
    ...
    def _validate_create_request(self, req, item_id, context):
        ...
        if not (1 <= req.poder_de_objeto <= 1000):
            context.abort(grpc.StatusCode.OUT_OF_RANGE, "Poder debe estar entre 1 y 1000")
        if len(req.gemas) > 2:
        ...
        if req.tipo == diablo_inventory_pb2.ItemType.WEAPON:
            ...
            context.abort(grpc.StatusCode.INVALID_ARGUMENT, "Debe especificarse dano o armadura")
